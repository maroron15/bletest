...
{ 
    ...
    HAL_GPIO_WritePin(GPIOF,GPIO_PIN_13,GPIO_PIN_RESET); //BCTS 0
     ...
     //아래 uart3경우랑 달리 switch문 안쓰고 sprintf((char *)TmpBle,"BLE:I'm%c\r\n",TmpBleCase); 하고 바로 전성.
	...
	if((huart->Instance)==USART3){//uart - pc. receive from pc //to mcu
	  ...
	  HAL_GPIO_WritePin(GPIOF,GPIO_PIN_12,GPIO_PIN_RESET); //BRTS 0
		...
		switch(TmpPcCase){ //사실 이 상황에서는 switch문 안써도됨 숫자대신 TmpPcCase 넣어주면 됨. 위에 uart6일 때 케이스처럼
			case '0':
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
		...
		HAL_UART_Receive(&huart6,rec_from_BLE,14,30000); //wait receive connected message for 30sec
  ...
  /* USER CODE BEGIN 3 */
	  ...
	  HAL_GPIO_WritePin(GPIOF,GPIO_PIN_12,GPIO_PIN_SET); //BRTS 1 
		*/
		...
		HAL_GPIO_WritePin(GPIOF,GPIO_PIN_12,GPIO_PIN_SET); //BRTS 1
		HAL_GPIO_WritePin(GPIOF,GPIO_PIN_13,GPIO_PIN_SET); //BCTS 1
		HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_7); //Toggle ld2. blue color led
		HAL_Delay(500);
  }
  /* USER CODE END 3 */
